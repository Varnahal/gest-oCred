  ...
  try {
    ...
    if (!apiKey) {
      return res.status(400).json({ 
        error: "A chave de API do Groq (GROQ_API_KEY) não está configurada no servidor do Vercel. Por favor, acione a aba de 'Environmental Variables' nas configurações do seu projeto no menu do Vercel e adicione essa variável com seu token do Groq."
      });
    }
...
${borrowers.length === 0 ? '- Nenhum cliente cadastrado ainda no banco de dados.' : borrowers.map((b) => `- ID: ${b.id}\n  Nome: ${b.name}\n  Documento: ${b.document || 'Não informado'}\n  Telefone: ${b.phone || 'Não informado'}\n  E-mail: ${b.email || 'Não informado'}`).join('\n\n')}

LISTA DE EMPRÉSTIMOS CONCEDIDOS (${loans.length} contratos):
${loans.length === 0 ? '- Nenhum contrato de empréstimo ativo no momento.' : loans.map((l) => {
  const borrowerName = borrowers.find((b) => b.id === l.borrower_id)?.name || 'Cliente Desconhecido';
...
==========================================`;
    ...
    console.error("Error at /api/chat serverless:", error);
